...
Button_PinState_e Button_GetPinState(Button_Config_t *inout_ptButton) {
  ...
  GPIO_PinState ePinState =
      HAL_GPIO_ReadPin(inout_ptButton->pGpioPort, inout_ptButton->u16GpioPin);
...
void Button_GetDebouncedState(Button_Config_t *const inout_ptButton) {
  ...
  case BUTTON_DEBOUNCE_RELEASE_MASK: {
    // Button Released -> Zeros are shifted in (16 Msec consistent shifting of
    // zeros) Clear the current long button press progress so that it does not
