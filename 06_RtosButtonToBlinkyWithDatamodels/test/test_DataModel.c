...
void setUp(void) { g_bObserverNotified = false; }

void tearDown(void) {}

void TestSupport_ObserverCallback(void) { g_bObserverNotified = true; }
...
void test_DataModel_Add_and_Remove_Observer(void) {
  ...
  u32 u32NofObserversBefore =
      DataModel_GetNumberOfAttachedObservers(&tTestDataModel);
...
void test_DataModel_Write_WritesValueToContentWhichCanThenBeRead(void) {
  DataModel_t tTestDataModel = {0};
  DataModel_Init(&tTestDataModel);
  Observer_t tObserver = {0};
  tObserver.u32Id = 1;
  ...
  char acBuffer[5] = {0};
  u32 u32DataSize = sizeof(acBuffer);
  DataModel_Read(&tTestDataModel, acBuffer, &u32DataSize);
  TEST_ASSERT_EQUAL_STRING("Test", acBuffer);
}
