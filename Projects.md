...
- CLI: 
  ...
  - Write a command (or several), which plays music on the buzzer
...
- The PWM Signal shall be generated by the timer peripheral. The timer settings can be manually updated - meaning we do not require an isr or a dma for that matter

### 22_EncoderToRgbLed
- Use the Rotary RGB Click Board
- Inputs
  - Capture the Rotary Encoder's information
  - Capture the Rotary Encoder's Button and extend the existing Button Driver so that now the Rotary Encoder can be handled in the same fashion
...
- Input -> Output
  - Write an example application, which translates the encoder ticks to the position of where 3 leds are powered on (the three leds are right next to each other)
  - pressing the Encoder changes the color of all enabled leds from red -> blue -> green -> off -> red -> ...
...
- CLI: Write a PID Controller, by which you can control the temperature of the MCU

### 32_ReadTheMic
- Use the DMA to run high-speed sampling of the analog microphone
- Hold the Button to record a 5 second sample into the RAM
- VORSICHT: NOCHMAL ÜBERARBEITEN - FINDE HERAUS UM WAS ES HIER GEHT!!
  - Im wesentlichen soll es hier um die Notwendigkeit von High-Speed DMA Transfers gehen.
- After the Recording try to replicate the sound on the previously created buzzer driver (Use your knowledge of DSP for that matter - have a look here: https://www.youtube.com/watch?v=uBEL3YVzMwk and 3blueOneBrown's YT Channel) 

## 4. Sensors and Actuators (SPI and I2C)
- Ideen:
- Man könnte hier auch 2 I2C Peripherals miteinander sprechen lassen (auf dem gleichen Microcontroller), aber ganz gezielt Störungen einbauen. 
